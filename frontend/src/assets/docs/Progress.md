...
- **Icon Resolution:** Fixed `check-circle-2` and `x-circle` resolution errors by converting them to the correctly supported `circle-check` and `circle-x` Lucide aliases.
